stock LoadConfig()
{
	...
	if(file_exists(g_szLogName))
	{
		...
		Util_Log("ZB Log Start: Previous log deleted.")
	}
	...
	while (file && !feof(file))
	{
		...
		switch (section)
		{
			...
			{
				if (equal(key, "ZB_LV1_HEALTH"))
					ZB_LV1_HEALTH = str_to_num(value)
				else if (equal(key, "ZB_LV1_ARMOR"))
					ZB_LV1_ARMOR = str_to_num(value)
				if (equal(key, "ZB_LV2_HEALTH"))
					ZB_LV2_HEALTH = str_to_num(value)
				else if (equal(key, "ZB_LV2_ARMOR"))
					ZB_LV2_ARMOR = str_to_num(value)
				if (equal(key, "ZB_LV3_HEALTH"))
					ZB_LV3_HEALTH = str_to_num(value)
				else if (equal(key, "ZB_LV3_ARMOR"))
					ZB_LV3_ARMOR = str_to_num(value)
				else if (equal(key, "ZB_HOST_HEADSHOT"))
				{
					new i = 0;
					while (value[0] != 0 && strtok(value, key, charsmax(key), value, charsmax(value), ','))
					{
						...
						trim(key);
						trim(value);
						c_flHostHead[i] = str_to_float(key);
						i++;
					}
				}
				else if (equal(key, "ZB_HOST_SPEEDUP"))
				{
					new i = 0;
					while (value[0] != 0 && strtok(value, key, charsmax(key), value, charsmax(value), ','))
					{
						...
						trim(key);
						trim(value);
						c_flHostSpeed[i] = str_to_float(key);
						i++;
					}
				}
				else if (equal(key, "ZB_HOST_JUMPUP"))
				{
					new i = 0;
					while (value[0] != 0 && strtok(value, key, charsmax(key), value, charsmax(value), ','))
					{
						...
						trim(key);
						trim(value);
						c_flHostJump[i] = str_to_float(key);
						i++;
					}
				}
				else if (equal(key, "RESPAWN_TIME_WAIT"))
					RESPAWN_TIME_WAIT = str_to_num(value)
				else if (equal(key, "ALIVE_TERM"))
					ALIVE_TERM = str_to_num(value)
				else if (equal(key, "FORCERESPAWN_TIME"))
					FORCERESPAWN_TIME = str_to_num(value)
			}
			case SECTION_HERO:
			{
			...
			case SECTION_ZOMBIEBOM:
			{
				if (equal(key, "RADIUS"))
					...
					ZOMBIEBOM_DAMAGE = str_to_float(value);
			}
			case SECTION_SOUNDS:
			{
				if (equal(key, "ENABLE_LEVELUP_SOUND"))
					levelupsound = str_to_num(value)
				else if (equal(key, "ZOMBIE_COMING"))
				{
					while (value[0] != 0 && strtok(value, key, charsmax(key), value, charsmax(value), ','))
					{
						trim(key)
...
stock LoadBoxSpawns()
{
	...
	if (file_exists(filepath))
	{
		...
		while (file && !feof(file))
		{
			...
			if (g_spawnCount_box >= sizeof g_spawns)
				break;
		}
		if (file)
			fclose(file)
			...
			case SECTION_HERO:
			{
				if (equal(key, "HERO_ITEMS"))
				{
					...
					while (value[0] != 0 && strtok(value, key, charsmax(key), value, charsmax(value), ','))
					{
						trim(key)
						trim(value)
						Util_Log("Load Hero Weapon: %s", key)
						ArrayPushString(HERO_ITEMS, key)
					}
				}
				if (equal(key, "HERO_HEALTH"))
				{
					HERO_HEALTH = str_to_num(value);
				}
			}
			case SECTION_REWALK:
			{
				if (equal(key, "SIDEKICK_ITEMS"))
				{
					strtolower(value)
					while (value[0] != 0 && strtok(value, key, charsmax(key), value, charsmax(value), ','))
					{
						trim(key)
						trim(value)
						Util_Log("Load Sidekick Weapon: %s", key)
						ArrayPushString(SIDEKICK_ITEMS, key)
					}
				}
				else if (equal(key, "MASTER_OF_COMBAT_ATK"))
				{
					ATK_MASTEROFCOMBAT = str_to_num(value);
				}
				else if (equal(key, "MORALE_BOOST_ATK"))
				{
					ATK_MORALEBOOST = str_to_num(value);
				}
				else if (equal(key, "MORALE_BOOST_MAX"))
				{
					MAX_MORALEBOOST = str_to_num(value);
				}
				else if (equal(key, "MASTER_OF_COMBAT_MAX"))
				{
					MAX_MASTEROFCOMBAT = str_to_num(value);
				}
			}
		}
	}
